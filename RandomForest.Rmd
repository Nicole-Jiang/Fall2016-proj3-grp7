...
```{r}
library(randomForest)
rf=randomForest(rf.form,train_data,ntree=500,importance=T)
plot(rf)
```
500 decision trees or a forest has been built using the Random Forest algorithm based learning. We can plot the error rate across decision trees. Black solid line is OOB error rate and a bunch of colour lines, one for each class' error
*OOB is the mean prediction error on each training sample x???, using only the trees that did not have x??? in their bootstrap sample. The plot seems to indicate that after 300 decision trees, there is not a significant reduction in error rate.
